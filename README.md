...
| **Storyboard** | `/storyboard` | 字コンテ（テキスト台本）を解析してカット表に分解し、絵コンテ画像を一括生成する。手書きスケッチをレイアウト参照として各カットに添付し、構図を保ったまま清書することもできる |
...
app/
  ...
  api/
    ...
    storyboard/
      ...
      qa/               生成画像の自己採点（文字混入・ト書き一致・キャラ一致・スタイル）
...
lib/
  ...
  storyboard/
    ...
    prompt.ts           プロンプト組み立て（段落構造の自然文。アングル/サイズ/構図/照明/レンズ辞書 + スタイルプリセット）
    guard.ts            実在人名ガード（API送信直前の最終ゲート）
    qa.ts               AIチェックの判定（ok / warn / retry）
...
- **手書きスケッチはレイアウト参照**。カットに添付したスケッチは入力画像 `@in1` として送られ、
  プロンプトの Layout 段落で「構図・配置を保って清書」を指示する（踏襲度は 厳密／ゆるく の2段階）
- **生成後にAIチェック**。視覚モデルで採点し、文字混入やト書き不一致が著しい場合は
  1回だけ自動で再生成する（`lib/storyboard/qa.ts`）。ON/OFF は画面下のチェックボックス
- **プロンプトは自然文**。Gemini / GPT Image の公式指針に合わせ、キーワード列挙ではなく
  場面を描写する段落構造で送る。生成前に右パネルで実際の送信文を確認できる
